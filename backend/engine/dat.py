...
def generate_mileage(current_mileage, status):
    if status == "revenue":
        return current_mileage + random.uniform(100, 500)
...
def simulate_outcomes(status, fitness_valid, branding_hours, mileage_deviation):
    punctuality = 1.0 if status == "revenue" and fitness_valid else random.uniform(0.95, 1.0) if status == "revenue" else None
    cost = random.uniform(1000, 5000) if status == "maintenance" else 0
...
def generate_train_induction_dataset():
    data = []
    trainsets = {i: {"mileage": random.uniform(10000, MAX_MILEAGE), "branding_total": 0} for i in range(1, NUM_TRAINSETS + 1)}
    fitness_expiry = {}
    job_card_expiry = {}
    ...
    for day in range(NUM_DAYS):
        ...
        for train_id in daily_trains:
            ...
            trainsets[train_id]["branding_total"] += branding_hours if job_card == "closed" else 0
            mileage = trainsets[train_id]["mileage"]
            
            score = heuristic_induction_score(fitness, job_card, branding_hours, mileage, mean_mileage, None)
            daily_data.append({
                "train_id": train_id,
                "fitness": fitness,
                "job_card": job_card,
    ...
    df["branding_sla_met"] = df["branding_hours"] >= BRANDING_SLA_MIN_HOURS
    df["mileage_balance_deviation"] = df.groupby("date")["mileage"].transform(lambda x: abs(x - x.mean()))
