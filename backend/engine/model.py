...
class OptiInductChatbot:
    def __init__(self, model_name="gemini-pro-latest"):
        """Initializes the chatbot with the Gemini model and sets up the API key."""
        self.induction_list = None
        self.chat_session = None

        api_key = os.getenv("GEMINI_API_KEY")
        if not api_key:
            raise ValueError("⚠️ GEMINI_API_KEY not found. Please set it in .env or environment variables.")

        genai.configure(api_key=api_key)
        self.model = genai.GenerativeModel(model_name)

    def set_induction_list(self, induction_list):
        """
        Sets the induction list and initializes a new chat session.
        This provides the model with the necessary context at the start.
        """
        self.induction_list = induction_list
        # Start a new chat session with the induction list as initial context
        self.chat_session = self.model.start_chat(
            history=[
                # Provide the induction list as a starting point for the conversation
                {"role": "user", "parts": [f"Please use the following induction plan to answer all my questions:\n{self.induction_list}"]},
                {"role": "model", "parts": ["Understood. I'm ready to answer questions about the provided induction plan."]}
            ]
        )
    ...
    def chat(self, user_input):
        ...
        The session automatically manages and sends the history.
        """
        if not self.chat_session:
            return "Please run optimization first to set the induction plan and start the chat."

        # Send the user's message to the chat session
        response = self.chat_session.send_message(user_input)

        # The chat history is now managed by the chat_session object
        return response.text

    @property
    def chat_history(self):
        """Provides a convenient way to access the chat session's history."""
        # The history is stored as "role" and "parts" in the session,
        # so we convert it to your desired format ("user", "message").
        history_list = []
        if self.chat_session:
            for message in self.chat_session.history:
                role = message.role
                text = message.parts[0].text
                # Convert the role names to match your display logic
                display_role = "user" if role == "user" else "assistant"
                history_list.append((display_role, text))
        return history_list